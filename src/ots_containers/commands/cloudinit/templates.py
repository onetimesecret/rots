...
SECRET_VARIABLE_NAMES=STRIPE_API_KEY,STRIPE_WEBHOOK_SIGNING_SECRET,SECRET,SESSION_SECRET,HMAC_SECRET,SMTP_PASSWORD
...
def generate_cloudinit_config(
    *,
    ...
    - podman socket enabled and ``ots-containers init`` invoked via runcmd
    ...
    runcmd: list[str] = [
        ...
        # Enable podman socket so ots-containers can manage containers
        "systemctl enable --now podman.socket",
        # Install ots-containers CLI then run init to scaffold the deployment DB
        "pip3 install ots-containers",
        "ots-containers init",
    ]
